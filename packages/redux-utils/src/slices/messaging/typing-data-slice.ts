...
export const typingDataSlice = createSlice({
    ...
    initialState,
    reducers: {
        typingStarted: (state, action: PayloadAction<{ channelSid: string, participant: string }>) => {
            const { channelSid, participant } = action.payload;
            const existedUsers = state[channelSid] ?? [];
            //existedUsers.push(participant);
        },
        typingEnded: (state, action: PayloadAction<{ channelSid: string, participant: string }>) => {
            const { channelSid, participant } = action.payload;
            const index = (state[channelSid] ?? []).findIndex(user => user === participant);
            if (index !== -1) {
